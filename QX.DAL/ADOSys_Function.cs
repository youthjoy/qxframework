...
namespace QX.DAL
{
   ...
   public partial class ADOSys_Function
   {
      ...
      {
         string sql = "INSERT INTO Sys_Function (Fun_RCode,Fun_Code,Fun_Name,Fun_PCode,Fun_PName,Fun_Order,Fun_iType,Fun_UDef1,Fun_UDef2,Fun_UDef3,Fun_UDef4,Fun_UDef5,Fun_UDef6,Fun_Stat,Stat,CreateDate,UpdateDate,DeleteDate) VALUES (@Fun_RCode,@Fun_Code,@Fun_Name,@Fun_PCode,@Fun_PName,@Fun_Order,@Fun_iType,@Fun_UDef1,@Fun_UDef2,@Fun_UDef3,@Fun_UDef4,@Fun_UDef5,@Fun_UDef6,@Fun_Stat,@Stat,@CreateDate,@UpdateDate,@DeleteDate)";
         ...
         if (string.IsNullOrEmpty(sys_Function.Fun_PName))
         {
            idb.AddParameter("@Fun_PName", DBNull.Value);
         }
         else
         {
            idb.AddParameter("@Fun_PName", sys_Function.Fun_PName);
         }
      ...
      if(sys_Function.Fun_PName_IsChanged){sbParameter.Append("Fun_PName=@Fun_PName, ");}
      ...
      string sql=sb.ToString();
         ...
         if(sys_Function.Fun_PName_IsChanged)
         {
         if (string.IsNullOrEmpty(sys_Function.Fun_PName))
         {
            idb.AddParameter("@Fun_PName", DBNull.Value);
      ...
      public Sys_Function GetByKey(decimal fun_ID)
      {
         ...
         string sql = "SELECT  Fun_ID,Fun_RCode,Fun_Code,Fun_Name,Fun_PCode,Fun_PName,Fun_Order,Fun_iType,Fun_UDef1,Fun_UDef2,Fun_UDef3,Fun_UDef4,Fun_UDef5,Fun_UDef6,Fun_Stat,Stat,CreateDate,UpdateDate,DeleteDate FROM Sys_Function WHERE 1=1 AND ((Stat is null) or (Stat=0) )  AND Fun_ID=@Fun_ID ";
         idb.AddParameter("@Fun_ID", fun_ID);
         ...
         while(dr.Read())
         {
            ...
            if (dr["Fun_PName"] != DBNull.Value) sys_Function.Fun_PName = Convert.ToString(dr["Fun_PName"]);
      ...
      public List<Sys_Function> GetListByWhere(string strCondition)
      {
         ...
         string sql = "SELECT  Fun_ID,Fun_RCode,Fun_Code,Fun_Name,Fun_PCode,Fun_PName,Fun_Order,Fun_iType,Fun_UDef1,Fun_UDef2,Fun_UDef3,Fun_UDef4,Fun_UDef5,Fun_UDef6,Fun_Stat,Stat,CreateDate,UpdateDate,DeleteDate FROM Sys_Function WHERE 1=1 AND ((Stat is null) or (Stat=0) ) ";
         if(!string.IsNullOrEmpty(strCondition))
         {
            strCondition.Replace('\'','"'); //防sql注入
            sql += strCondition ;
         }
          sql += " ORDER BY Fun_ID DESC "; 
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
      ...
      {
         List<Sys_Function> ret = new List<Sys_Function>();
         string sql = "SELECT  Fun_ID,Fun_RCode,Fun_Code,Fun_Name,Fun_PCode,Fun_PName,Fun_Order,Fun_iType,Fun_UDef1,Fun_UDef2,Fun_UDef3,Fun_UDef4,Fun_UDef5,Fun_UDef6,Fun_Stat,Stat,CreateDate,UpdateDate,DeleteDate FROM Sys_Function where 1=1 AND ((Stat is null) or (Stat=0) ) order by Fun_ID desc ";
          SqlDataReader dr=null;  
           try {  
          dr=(SqlDataReader)idb.ReturnReader(sql);
         while(dr.Read())
